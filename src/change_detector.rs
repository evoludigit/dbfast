...
impl ChangeDetector {
    ...
    pub async fn template_needs_rebuild(&self, template_name: &str) -> Result<bool, ChangeDetectionError> {
        // Get stored metadata for the template
        let stored_metadata = match self.get_template_metadata(template_name).await? {
        ...
        self.compare_files(&current_files, &stored_metadata)
    }
    ...
    pub async fn store_template_metadata(
        ...
        let metadata_file = self.metadata_dir.join(format!("{}.json", template_name));
    ...
    fn compare_files(
        &self,
        current_files: &[ScannedFile],
        stored_metadata: &[ScannedFile],
    ) -> Result<bool, ChangeDetectionError> {
        // Quick check: different number of files
        if current_files.len() != stored_metadata.len() {
            return Ok(true); // Files added/deleted, rebuild needed
        }
        
        // Create hash maps for efficient comparison
        let current_map: HashMap<&PathBuf, &String> = current_files
            .iter()
            .map(|f| (&f.path, &f.hash))
            .collect();
            
        let stored_map: HashMap<&PathBuf, &String> = stored_metadata
            .iter()
            .map(|f| (&f.path, &f.hash))
            .collect();
            ...
            match stored_map.get(path) {
                Some(stored_hash) => {
                    if current_hash != stored_hash {
                        return Ok(true); // File modified, rebuild needed
                    }
                }
                None => return Ok(true), // New file, rebuild needed
            }
        }
        ...
        for path in stored_map.keys() {
            if !current_map.contains_key(path) {
                return Ok(true); // File deleted, rebuild needed
            }
        }
