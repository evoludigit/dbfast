    ...
    match DatabasePool::new(&config.database).await {
        ...
        Err(_) => {
            ...
            let validation_result =
                CloneManager::validate_database_name("template'; DROP DATABASE important_data; --");
            assert!(
                matches!(
                    validation_result,
                    Err(CloneError::InvalidDatabaseName { .. })
...
async fn test_sql_injection_protection_in_clone_name() {
    ...
    match DatabasePool::new(&config.database).await {
        ...
        Err(_) => {
            ...
            let validation_result =
                CloneManager::validate_database_name("clone'; CREATE DATABASE malicious_db; --");
            assert!(
                matches!(
                    validation_result,
                    Err(CloneError::InvalidDatabaseName { .. })
    ...
    match DatabasePool::new(&config.database).await {
        Ok(pool) => {
            ...
            for invalid_name in invalid_names {
                let result = clone_manager.clone_database("template", invalid_name).await;
