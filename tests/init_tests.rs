...
fn test_init_command_creates_config() {
    ...
    assert!(stdout.contains("Successfully initialized"), "stdout was: {}", stdout);
...
fn test_init_function_with_invalid_repo_dir() {
    ...
    assert!(error.to_string().contains("Repository directory does not exist"));
}
...
fn test_init_command_with_invalid_repo_dir() {
    ...
    assert!(stderr.contains("Repository directory does not exist"), "stderr was: {}", stderr);
}