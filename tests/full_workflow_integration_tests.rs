...
async fn test_full_workflow_file_scanning_to_change_detection() {
    ...
    for i in 1..initial_scan.len() {
        assert!(
            initial_scan[i - 1].path <= initial_scan[i].path,
    ...
    assert!(
        ...
        "Change detection should be <50ms, was {}ms",
        duration.as_millis()
    );
}
...
async fn test_full_workflow_with_file_addition_and_deletion() {
    ...
    // Add a new file
    let new_file_dir = temp_dir
        .path()
        .join("0_schema/01_write_side/010_blog/0103_comment");
    ...
    assert!(
        !needs_rebuild,
        "Should not need rebuild after metadata update"
    );
...
async fn test_workflow_performance_with_many_files() {
    ...
    assert!(
        ...
        "File scanning should be <100ms, was {}ms",
    ...
    assert!(
        change_duration.as_millis() < 50,
        "Change detection should be <50ms even with changes, was {}ms",
        change_duration.as_millis()
    );
}
