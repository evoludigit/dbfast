...
use dbfast::config::{Config, DatabaseConfig, RepositoryConfig, Environment};
...
fn create_test_project_with_environments() -> TempDir {
    ...
    environments.insert("local".to_string(), Environment {
    ...
    environments.insert("production".to_string(), Environment {
        include_directories: vec![
            "0_schema".to_string(),
            "1_seed_common".to_string(),
        ],
...
fn test_environments_command_lists_configured_environments() {
    ...
    assert!(stdout.contains("production"), "Should show production environment");
    ...
    assert!(stdout.contains("0_schema"), "Should mention schema directory");
...
fn test_environments_command_shows_different_file_counts() {
    ...
    let production_line = stdout.lines().find(|line| line.contains("production")).unwrap();
    
    assert!(local_line.contains("files"), "Local environment should show file count");
    assert!(production_line.contains("files"), "Production environment should show file count");
    
    // Both should have file counts, but they should be different
    // (this is a basic check - we'll verify exact counts in unit tests)
    assert_ne!(local_line, production_line, "Local and production should have different configurations");
}
...
fn test_environments_command_verbose_mode() {
    ...
    assert!(stdout.contains("include") || stdout.contains("Include"), "Should show include patterns");
    assert!(stdout.contains("exclude") || stdout.contains("Exclude"), "Should show exclude patterns");
...
fn test_validate_env_command_with_valid_environment() {
    ...
    assert!(output.status.success(), "Valid environment should pass validation");
...
fn test_validate_env_command_with_invalid_environment() {
    ...
    assert!(!output.status.success(), "Invalid environment should fail validation");
...
fn test_enhanced_status_command_shows_environment_info() {
    ...
    assert!(stdout.contains("production"), "Should list production environment");
}
...
fn test_enhanced_status_verbose_shows_detailed_environment_info() {
    ...
    assert!(stdout.contains("files"), "Should show file counts per environment");
    assert!(stdout.contains("local"), "Should show local environment details");
    assert!(stdout.contains("production"), "Should show production environment details");
    
    // Should be more verbose than regular status
    assert!(stdout.len() > 300, "Verbose status should provide detailed information");
}
