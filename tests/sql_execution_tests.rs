...
fn test_sql_file_reading() {
    ...
    assert!(
        !statements.is_empty(),
        "Should find SQL files in db directory"
    );

    // Should find the user table creation statement
    let has_user_table = statements
        .iter()
        .any(|stmt| stmt.contains("CREATE TABLE blog.tb_user"));
...
fn test_sql_file_ordering() {
    ...
    let schema_pos = statements
        .iter()
        .position(|stmt| stmt.contains("CREATE SCHEMA"));
    let table_pos = statements
        .iter()
        .position(|stmt| stmt.contains("CREATE TABLE"));
...
async fn test_sql_execution_against_database() {
    ...
    assert!(
        result.is_ok(),
        "Should be able to execute SQL statements against database: {:?}",
        result.err()
    );

    // Verify the data was inserted
    let count_result = pool
        .query(
            &format!("SELECT COUNT(*) FROM {}.test_table", test_schema),
            &[],
        )
        .await;
    assert!(
        count_result.is_ok(),
        "Should be able to query the created table: {:?}",
        count_result.err()
    );

    // Clean up: drop the test schema
    let cleanup_result = pool
        .execute(&format!("DROP SCHEMA {} CASCADE", test_schema), &[])
        .await;
    assert!(
        cleanup_result.is_ok(),
        "Should be able to clean up test schema"
    );
}
