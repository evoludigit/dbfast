    ...
    match DatabasePool::new(&config.database).await {
        Ok(pool) => {
            ...
            for i in 0..5 {
                ...
                match clone_manager
                    .clone_database("blog_template", &clone_name)
                    .await
                {
                    Ok(()) => {
                    ...
                    Err(_) => {
                        println!(
                            "⚠️  Clone {} failed (expected without PostgreSQL server)",
                            i
                        );
        ...
        Err(_) => {
            println!(
                "⚠️  No database connection for performance test (expected without PostgreSQL)"
            );
        }
    }
}
...
async fn test_concurrent_database_cloning_performance() {
    ...
    match DatabasePool::new(&config.database).await {
        Ok(pool) => {
            ...
            println!(
                "Concurrent cloning completed in {}ms",
