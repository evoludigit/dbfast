    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "Should be a database/config related error, got: {}", error_message
            );
        }
    }
}
...
async fn test_seed_command_missing_config() {
    ...
    assert!(error.contains("dbfast.toml"), "Should mention config file: {}", error);
...
"#;
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(!error_str.contains("runtime"), "Should not be runtime error: {}", error_str);
        }
    }
}