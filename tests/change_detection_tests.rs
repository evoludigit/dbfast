...
async fn test_change_detection_with_no_template_metadata() {
    ...
    assert!(
        needs_rebuild,
        "Should need rebuild when no template metadata exists"
    );
}
...
async fn test_change_detection_with_matching_metadata() {
    ...
    assert!(
        !needs_rebuild,
        "Should not need rebuild when files haven't changed"
    );
}
...
async fn test_change_detection_with_modified_file() {
    ...
    assert!(
        needs_rebuild,
        "Should need rebuild when file has been modified"
    );
}
...
async fn test_change_detection_with_new_file() {
    ...
    // Add a new SQL file
    let schema_dir = temp_dir
        .path()
        .join("0_schema/01_write_side/010_blog/0101_user");
    let new_file = schema_dir.join("010113_new_constraint.sql");
    fs::write(
        &new_file,
        "ALTER TABLE tb_user ADD CONSTRAINT ck_name_not_empty CHECK (name != '');",
    )
    .unwrap();
    ...
    assert!(
        needs_rebuild,
        "Should need rebuild when new file has been added"
    );
}
...
async fn test_change_detection_with_deleted_file() {
    ...
    assert!(
        needs_rebuild,
        "Should need rebuild when file has been deleted"
    );
}
...
async fn test_template_metadata_storage_and_retrieval() {
    ...
    assert_eq!(
        metadata.len(),
        scanned_files.len(),
        "Retrieved metadata should match stored data"
    );
