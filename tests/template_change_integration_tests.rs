...
async fn test_create_template_with_change_tracking() {
    ...
    assert!(result.is_ok(), "Template creation with change tracking should succeed");
    ...
    assert!(metadata.is_some(), "Change detection metadata should be stored after template creation");
}
...
async fn test_template_needs_rebuild_integration() {
    ...
    assert!(!needs_rebuild, "Template should not need rebuilding after creation");
    ...
    assert!(needs_rebuild, "Template should need rebuilding after file modification");
}
...
async fn test_smart_template_creation() {
    ...
    assert!(result.is_ok());
    assert!(result.unwrap(), "Should return true when template was actually created");
    ...
    assert!(result.is_ok());
    assert!(!result.unwrap(), "Should return false when template creation was skipped");
    ...
    assert!(result.is_ok());
    assert!(result.unwrap(), "Should return true when template was rebuilt due to changes");
}
